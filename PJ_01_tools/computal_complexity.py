...
def cnn_complexity(input_channel,output_channel,kernel_size,output_dimension, stride):
    sum_encoder = 0
    for i in xrange(len(input_channel)):
        x1 = (kernel_size[0] * kernel_size[1]) * input_channel[i] * 1 * output_dimension[
            i] * output_channel[i]
        if mode == Mode.my_crd and i == 0 or  mode==Mode.my_crd_width and i == 0:
            #first layer hat kernel size(1,1)
            x1 = x1/(kernel_size[0] * kernel_size[1])
        #print(x1)
        sum_encoder += x1
    return sum_encoder
...
def rnn_complexity(input_dimension, neuron_number):
    sum_rnn = 0
    for i in xrange(len(input_dimension)):
        x = 4 * (neuron_number[i]**2 + neuron_number[i]*input_dimension[i])
        sum_rnn += x
    return sum_rnn
...
def dense_complexitiy(input_dimension, neuron_number):
    sum_dense = 0
    for i in range(len(input_dimension)):
        x = input_dimension[i]*neuron_number[i]
        sum_dense += x
    return sum_dense
...
if __name__ == '__main__':
    ...
    if mode == Mode.cruse or mode== Mode.microsoft:
        ...
        if mode == Mode.microsoft:
            ...
            skip_connection = cnn_complexity(input_channel=input_channel_skip, output_channel=output_channel_skip,
                                         kernel_size=skip_kernel, output_dimension=output_dimension_skip, stride=[1, 1])
    ...
    # print(enc_mos)
    # #print(rnn_mos)
    # print('encoder and decoder:',dec_mos)
    # # print('skip',skip_connection)
    # # print('all:',rnn_mos +enc_mos +dense_complexitiy())
