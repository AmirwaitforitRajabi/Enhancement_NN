...
result_path = pathlib.Path(
    f'F:/Projekts/SE/Phase_Korrektur/big-Timit-data/w1,w2/01_crd_model_{batch_size}_w1_{w1}_w2_{w2}_w3_{w3}_look_backward_{look_backward}_{learning_rate}_return_true_last_ReLU_my_methode_data_casual')
startAll = time.time()
...
def custom_loss(y_true, y_predict):
    ...
    # y_true_d_type = tf.cast(y_true_d_type, dtype=tf.double)
    # y_predict_d_type = tf.cast(y_predict_d_type, dtype=tf.double)
...
INPUT_Noisy_SHAPE = [look_backward + look_forward + 1, 257, 1]

input_noisy = Input(shape=INPUT_Noisy_SHAPE)
input_noise_component = Input(shape=(257,1), dtype=tf.complex64)
...
c1_reshape = tf.keras.layers.Reshape((look_backward + look_forward + 1, c4_activ_zero_padded.shape[2] * c4_activ_zero_padded.shape[3]))(
...
Mask = tf.cast(Output_Mask, tf.complex64)
