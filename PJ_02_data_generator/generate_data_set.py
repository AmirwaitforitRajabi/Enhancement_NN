...
import scipy.signal
import soundfile
from PJ01_tools.old.util import *
...
class SignalMixedProcessor:
    ...
    def process_snr_my_methode(self):
        # picked_noise_amp = self.picked_noise_amp[np.arange(len(self.clean_amp)) % len(self.picked_noise_amp)]
...
class DataAcquisition:
    ...
    def acquire_data(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(1) as executor:
            for clean in clean_path_wav:
                future = executor.submit(func_timeout, 6000, self._pick_noise_snr, (clean, noise_path_wav))
    ...
    def _pick_noise_snr(self, clean, noise_path_wav):
        if self.gen_mode == MixMethode.just_clean_data:
            in_data_amp, _ = librosa.load(clean, sr=16000)
            self.file_length = len(in_data_amp)
            #soundfile.write(self.clean_des_wav.joinpath(clean.name), in_data_amp, 16000, 'PCM_16')
            if self.Slv:
                for fft in self.fft:
                    complex_spec, abs_spec, phase = self._calculate_fft(in_data_amp,fft)
                    name = self.clean_des_slv.joinpath(clean.name[:-4]+".slv")
            ...
            for noise in noise_path_wav:
                clean_amp, _ = librosa.load(clean, sr=16000)
                noise_amp, _ = librosa.load(noise, sr=16000)
                self.file_length = len(clean_amp)

                for soll_snr in self.SNrs:
                    mixed_amp_gained = self._get_mixed_amplitude(clean_amp, noise_amp, soll_snr)

                    #corrcet the amplitude
                    # mixed_amp_gained_max = np.max(mixed_amp_gained)
                    # clean_amp_max = np.max(clean_amp)
                    # zigma = (clean_amp_max/mixed_amp_gained_max)*clean_amp
                    # soundfile.write(self.noisy_des_wav.joinpath(clean.name), zigma, 16000, 'PCM_16')
                    ...
                    if self.Slv:
                        for fft in self.fft:
                            ...
                            if fft == 320:
                                dest_slv = self.noisy_des_slv_0.joinpath(name + '.slv')
                                self.save_slv(complex_spec, abs_spec, phase, dest_slv, self.start, 'noisy')
                            else:
                                dest_slv = self.noisy_des_slv_1.joinpath(name + '.slv')
                                self.save_slv(complex_spec, abs_spec, phase, dest_slv, self.start, 'noisy')
    ...
    def _create_destination(self, destination):
        ...
        self.noisy_des_slv_0 = destination.joinpath('slv_'+f"{self.fft[0]}", 'noisy')
        self.noisy_des_slv_1 = destination.joinpath('slv_' + f"{self.fft[1]}", 'noisy')
        if not self.clean_des_slv.exists() \
                or not self.noisy_des_wav.exists() or not self.noisy_des_slv_0.exists() or not self.noisy_des_slv_1.exists():
            self.clean_des_slv.mkdir(parents=True, exist_ok=True)
            self.noisy_des_wav.mkdir(parents=True, exist_ok=True)
            self.noisy_des_slv_0.mkdir(parents=True, exist_ok=True)
            self.noisy_des_slv_1.mkdir(parents=True, exist_ok=True)
    ...
    def _enhance_asl(self, in_data,asl_soll):
        asl_ist = self.signal_power.calculate_active_speech_level(in_data,fs=16000)
    ...
    def _get_mixed_amplitude(self, clean_amp, noise_amp, soll_snr):
        if self.NoiseInit:
        ...
        if self.gen_mode == self.gen_mode.gent_mix_methode:
            ...
            return signal_mixer.process_snr_my_methode()
    ...
    def _save_enhanced_signal(self, name, amp):
        soundfile.write(name, amp, 16000, 'PCM_16')
    ...
    def save_slv(self, complex_spec, abs_spec, phase, name,start, x):
        ...
        g['noise_index'] = start
        if self.output_type == Mode.test_data:
            g['phase_' + x] = phase
